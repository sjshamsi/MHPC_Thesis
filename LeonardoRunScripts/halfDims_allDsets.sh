#!/bin/bash -l
#SBATCH --job-name=halfDims_allDsets
...
# `./submit.sh --export=ALL,AUTO_RESUME=False halfDims_allDsets.sh`) to force a
...
srun python -u `which torchrun` \
    ...
    --rdzv_endpoint=$SLURMD_NODENAME:29500 \
        train.py \
            ...
            name=halfDims_allDsets \
            trainer=defaults \
            trainer.grad_acc_steps=4 \
            optimizer=adam \
            optimizer.lr=2.e-4 \
            logger.wandb_project_name="halfDims_allDsets" \
