#!/bin/bash -l
#SBATCH --job-name=halfDims_activematter_BS1000
...
# `./submit.sh --export=ALL,AUTO_RESUME=False halfDims_activematter_BS1000.sh`) to force a
...
srun python -u `which torchrun` \
    ...
    --rdzv_endpoint=$SLURMD_NODENAME:29500 \
        train.py \
            ...
            name=halfDims_activematter_BS1000 \
            trainer=defaults \
            trainer.grad_acc_steps=4 \
            optimizer=adam \
            optimizer.lr=2.e-4 \
            logger.wandb_project_name="halfDims_activematter_BS1000" \
