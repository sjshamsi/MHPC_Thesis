#!/bin/bash -l
#SBATCH --job-name=halfDims_allDsets_tiny_video
...
# Same as halfDims_allDsets_tiny.sh (half-scale architecture: hidden_dim=1088,
# processor_blocks=30, mlp_dim=4352; same tiny/debug-queue shrink pattern), but with
...
# `./submit.sh --export=ALL,AUTO_RESUME=False halfDims_allDsets_tiny_video.sh`) to force a
...
srun python -u `which torchrun` \
    ...
    --rdzv_endpoint=$SLURMD_NODENAME:29500 \
        train.py \
            ...
            name=halfDims_allDsets_tiny_video \
            trainer=defaults \
            trainer.grad_acc_steps=4 \
            optimizer=adam \
            optimizer.lr=2.e-4 \
            logger.wandb_project_name="halfDims_allDsets_tiny_video" \
