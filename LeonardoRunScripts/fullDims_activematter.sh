#!/bin/bash -l
#SBATCH --job-name=fullDims_activematter
...
# val_frequency=10/etc. are all still unchanged from fullDims_allDsets.sh's throughput-
# experiment values, sized for an 8h normal-qos run, not a 30-minute debug slot. Even
# with only one (lighter, 2D) dataset instead of six, this will very likely hit
# boost_qos_dbg's TIME LIMIT well before max_epoch=71 - useful as a debug-queue
# calibration run (single-dataset case), same spirit as fullDims_allDsets_tiny.sh, but
# not sized to actually complete. Shrink max_epoch/max_samples/lr_scheduler ramps if you
# want a run that finishes within the slot.
...
# `./submit.sh --export=ALL,AUTO_RESUME=False fullDims_activematter.sh`) to force a
...
srun python -u `which torchrun` \
    ...
    --rdzv_endpoint=$SLURMD_NODENAME:29500 \
        train.py \
            ...
            optimizer=adam \
            optimizer.lr=2.e-4 \
            logger.wandb_project_name="fullDims_activematter" \
