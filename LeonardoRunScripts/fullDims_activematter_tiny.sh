#!/bin/bash -l
#SBATCH --job-name=fullDims_activematter_tiny
...
# check this run's own timing before deciding whether to raise it.
...
# `./submit.sh --export=ALL,AUTO_RESUME=False fullDims_activematter_tiny.sh`) to force a
...
srun python -u `which torchrun` \
    ...
    --rdzv_endpoint=$SLURMD_NODENAME:29500 \
        train.py \
            ...
            optimizer=adam \
            optimizer.lr=2.e-4 \
            logger.wandb_project_name="fullDims_activematter_tiny" \
