# ruff: noqa: E501
"""Generate single-node (1x boost_usr_prod node, 4x A100) SLURM scripts that star-search
...
Usage:
    python gen_star_search_runs.py
...
def main():
    ...
    for axis in options:
        ...
        for value in options[axis]:
            ...
            for lr in lrs:
                job_name = f"star{count}_hidden{hidden_dim}_depth{depth}_mlp{mlp}"
                ...
                script_path = os.path.join(SCRIPT_DIR, f"star_search_run_{count}.sh")
                with open(script_path, "w") as f:
                    f.write(script_body)
                os.chmod(script_path, 0o755)
                # SLURM won't create the %x subdirectory in --output/--error itself.
                os.makedirs(
                    os.path.join(EXPERIMENT_DIR, "slurm", job_name), exist_ok=True
                )
                scripts.append(f"sbatch star_search_run_{count}.sh")
                count += 1

    run_all_path = os.path.join(SCRIPT_DIR, "run_star_search_all.sh")
    with open(run_all_path, "w") as f:
        f.write("#!/bin/bash\n" + "\n".join(scripts) + "\n")
    os.chmod(run_all_path, 0o755)

    print(f"Generated {count} star search run scripts + run_star_search_all.sh in {SCRIPT_DIR}")
