#!/bin/bash -l
#SBATCH --job-name=fullDims_allDsets_tiny_video
...
# same paper-scale architecture and same data=available_leonardo (all 6 datasets), so
...
#     would never fire.
#
# Everything else (model.* architecture, data=available_leonardo,
...
# `./submit.sh --export=ALL,AUTO_RESUME=False fullDims_allDsets_tiny_video.sh`) to force a
...
srun python -u `which torchrun` \
    ...
    --rdzv_endpoint=$SLURMD_NODENAME:29500 \
        train.py \
            ...
            name=fullDims_allDsets_tiny_video \
            trainer=defaults \
            trainer.grad_acc_steps=4 \
            optimizer=adam \
            optimizer.lr=2.e-4 \
            logger.wandb_project_name="fullDims_allDsets_tiny_video" \
