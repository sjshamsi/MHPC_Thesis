#!/bin/bash -l
#SBATCH --job-name=halfDims_activematter_multinode
...
# stays within boost_qos_dbg's MaxTRESPerUser (node=8, gpu=32).
#
# Only the architecture changed here - trainer.max_epoch=71/max_samples=2000/
# val_frequency=10/etc. are all still unchanged from fullDims_activematter_multinode.sh's
...
# `./submit.sh --export=ALL,AUTO_RESUME=False halfDims_activematter_multinode.sh`) to
...
srun python -u `which torchrun` \
    ...
    --rdzv_endpoint=$SLURMD_NODENAME:29500 \
        train.py \
            ...
            optimizer=adam \
            optimizer.lr=2.e-4 \
            logger.wandb_project_name="halfDims_activematter_multinode" \
