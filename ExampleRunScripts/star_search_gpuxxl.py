...
SLURM_CPUS_ON_NODE = "96" #"{SLURM_CPUS_ON_NODE}"
...
\t\t--nnodes=5 \\
\t\t--nproc_per_node=8 \\
\t\t--rdzv_id=star{count}_lr{lr}_{depth}_hidden{hidden_dim}_mlp_{mlp} \\
\t\t--rdzv_backend=c10d \\
...
"""
            script_name = f"example_run_scripts/gpuxxl_star{count}_tuning_script_{count}.sh"
...
with open("example_run_scripts/gpuxxl_star_run_all.sh", "w") as f:
    f.write("\n".join(scripts))