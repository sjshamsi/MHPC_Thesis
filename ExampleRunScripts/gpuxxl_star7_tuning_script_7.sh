...
#SBATCH --output=logs/slurmid%j-star7_lr0.00025_12_hidden1408_mlp_5632.log
...
python -u `which torchrun` \
		--nnodes=5 \
		--nproc_per_node=8 \
		--rdzv_id=star7_lr0.00025_12_hidden1408_mlp_5632 \
		--rdzv_backend=c10d \
		--rdzv_endpoint=workergpu314:29500 \
			train.py distribution=hsdp name=star7_lr0.00025_12_hidden1408_mlp_5632 trainer.grad_acc_steps=4 server=gpuxl optimizer=adam optimizer.lr=0.00025 logger.wandb_project_name="MPPX_Scaling" \
